...
while [ $# -gt 0 ]; do
	case "$1" in
	--version)
		[ $# -ge 2 ] || err "Missing value for --version"
		VERSION="$2"
		[ -n "$VERSION" ] || err "Missing value for --version"
		shift 2
		;;
	--version=*)
		VERSION="${1#*=}"
	...
	--target)
		...
		TARGET="$2"
		[ -n "$TARGET" ] || err "Missing value for --target"
		shift 2
		;;
	--target=*)
		TARGET="${1#*=}"
		...
		;;
	--install-dir)
		[ $# -ge 2 ] || err "Missing value for --install-dir"
		INSTALL_DIR="$2"
		[ -n "$INSTALL_DIR" ] || err "Missing value for --install-dir"
		shift 2
		;;
	--install-dir=*)
		INSTALL_DIR="${1#*=}"
...
http_get() {
	# http_get <url> prints response body to stdout.
	if have curl; then
		curl -fsSL --retry 3 "$1"
	else
		wget -qO- "$1"
...
sha256() {
	# sha256 <path> prints the hex digest.
	...
	*) return 1 ;;
	...
	x86_64 | amd64)
		...
		if [ -r /proc/cpuinfo ]; then
			...
			# No /proc/cpuinfo. Assume emulation.
			return 0
		fi
		;;
	aarch64 | arm64)
...
TMP_BASE="${TMPDIR:-/tmp}"
TMP_DIR="$(mktemp -d "${TMP_BASE%/}/move-flow.XXXXXX" 2>/dev/null || mktemp -d -t move-flow.XXXXXX)"
cleanup() {
	[ -n "${TMP_DIR:-}" ] || return 0
	[ -d "$TMP_DIR" ] || return 0
	case "$TMP_DIR" in
	*/move-flow.*) rm -r "$TMP_DIR" ;;
	*) err "Refusing to remove unexpected temporary directory: $TMP_DIR" ;;
	esac
}
trap cleanup EXIT
